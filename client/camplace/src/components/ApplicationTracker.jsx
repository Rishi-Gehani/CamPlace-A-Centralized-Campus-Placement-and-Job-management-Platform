    ...
    <div className="w-full py-8">
      <div className="relative flex items-center justify-between">
        ...
        <motion.div 
          ...
          className={`absolute left-0 top-1/2 -translate-y-1/2 h-1 rounded-full ${isRejected ? 'bg-red-500' : 'bg-primary'}`}
        />
          ...
          return (
            <div key={stage.id} className="relative flex flex-col items-center gap-3 z-10">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-500 ${
                isCompleted ? (isRejected ? 'bg-red-200 text-red-600' : 'bg-primary text-white') : 
                isCurrent ? (isRejected ? 'bg-red-500 text-white shadow-lg shadow-red-200' : 'bg-white border-4 border-primary text-primary shadow-lg shadow-primary/20') :
                'bg-white border-2 border-black/5 text-secondary/20'
              }`}>
                {isCompleted ? <CheckCircle2 size={20} /> : 
                 isCurrent ? (isRejected ? <XCircle size={20} /> : <Clock size={20} className="animate-pulse" />) :
                 <Circle size={20} />}
              </div>
              <span className={`text-[10px] font-bold uppercase tracking-widest whitespace-nowrap ${
                isCurrent ? (isRejected ? 'text-red-500' : 'text-primary') : 
                'text-secondary/40'
              }`}>
                {stage.label}
              </span>
